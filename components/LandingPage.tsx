...
export default function LandingPage({ onEnter }: { onEnter: () => void }) {
  ...
  useEffect(() => {
    ...
    function draw() {
      ...
      if (sy > -50 && sy < H + 50) {
        ...
        ctx.save();
        const shadowLen = 120 + sunY_norm * 80;
        const shadowAngle = Math.PI - Math.atan2(cy - 60 - sy, cx - sx);
        ctx.strokeStyle = `rgba(160,140,110,${0.08 + sunY_norm * 0.06})`; ctx.lineWidth = 2;
        ctx.setLineDash([3, 8]);
        ctx.beginPath();
        ctx.moveTo(sx, sy);
        ctx.lineTo(cx, cy - 60);
        ctx.lineTo(cx + Math.cos(shadowAngle) * shadowLen, cy - 60 + Math.sin(shadowAngle) * shadowLen);
  ...
  const inView = (threshold = 0, extraScroll = 0) => ({
    opacity: scrollY > threshold ? 1 : 0,
    transform: scrollY > threshold ? 'translateY(0)' : 'translateY(24px)',
        ...
        <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 80, alignItems: 'center' }}>
          ...
          <div>
            {[
              ...
              ['The sun moves. A lot.','The difference between summer and winter sun angles can mean a room that's bright all day vs one that sees sunlight for two hours.'],
            ].map(([t,d],i)=>(
