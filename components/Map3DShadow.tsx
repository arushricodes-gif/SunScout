...
export default function Map3DShadow({ lat, lon, pathData, simTime, simPos, sunTimes, animating }: Map3DShadowProps) {
  ...
  const html = useMemo(() => {
    ...
    const pathCoords = JSON.stringify(pathData.filter(p => p.el >= 0).map(p => [p.lat, p.lon]));
...
.sun-label{background:linear-gradient(135deg,#F39C12,#E67E22);color:#000;padding:3px 9px;border-radius:7px;font-weight:700;font-size:11px;font-family:monospace;box-shadow:0 3px 10px rgba(243,156,18,.35);text-align:center;}
</style></head><body>
...
osmb.date(new Date('${simIso}'));
osmb.load('https://{s}.data.osmbuildings.org/0.2/59fcc2e8/tile/{z}/{x}/{y}.json');

L.circleMarker([${lat},${lon}],{radius:8,color:'#E07B00',weight:3,fillColor:'#E07B00',fillOpacity:0.9}).addTo(map);

var pathCoords=${pathCoords};
if(pathCoords.length>1){L.polyline(pathCoords,{color:'#F39C12',weight:4,opacity:0.85,dashArray:'8,10'}).addTo(map);}

var sunIcon=L.divIcon({html:'<div style="font-size:28pt;line-height:1;filter:drop-shadow(0 0 10px rgba(255,200,0,.9));">☀️</div><div class="sun-label" id="stl">--:--</div>',iconSize:[60,60],iconAnchor:[30,22],className:''});
var sunMarker=L.marker([${simPos.sunLat},${simPos.sunLon}],{icon:sunIcon}).addTo(map);
var shadowLine=L.polyline([[${lat},${lon}],[${simPos.shadowLat},${simPos.shadowLon}]],{color:'#8B9AB0',weight:4,dashArray:'6,8',opacity:0.7}).addTo(map);

var allPts=${allPtsJs};
function updateView(p){
  if(p.iso)osmb.date(new Date(p.iso));
...
var _saved=(function(){try{var s=sessionStorage.getItem('osmCam');return s?JSON.parse(s):{rot:0,tilt:0};}catch(e){return{rot:0,tilt:0};}})();
var curRot=_saved.rot||0,curTilt=_saved.tilt||0;
function saveCamera(){try{sessionStorage.setItem('osmCam',JSON.stringify({rot:curRot,tilt:curTilt}));}catch(e){}}
function applyCamera(){osmb.rotation(curRot);osmb.tilt(curTilt);document.getElementById('cmp').style.transform='rotate('+curRot+'deg)';}
applyCamera();
